...
> ⚠️ **2026-07-02 归因修正**：本文前提"这 100% 是手头问题"已被 T1 部分推翻——手分支零改动、只训 body 侧 `shape_out+cam_out` 就把 `[wa]` 0.229→0.204（反超 OSX），且改善**近端最大**（j1 −0.024 → tip −0.007）。腕对齐消平移不消尺度/掌部几何，其误差同样呈 distal 递增 → 本文的"distal 天花板支"读数大块其实是 **body 侧 shape/cam**；指尖残差（tip 0.256 vs OSX 0.263）才是真 articulation、归 decoder 线。分组归因**方法**仍有效（7-02 就用它做了 T1 的 size/side/finger/level 分析）。详见 `experiment_log.md` 2026-07-02。
